...
class Gene():
	def __init__(self, score_arr, mut_targ_arr):
		...
		# Updated in parse_VCF_lib/parse_VCF
		self.vars = []
...
class VariantCollection():
	def __init__(self, ENS_ID, Gene_obj_dict):
		...
		self.P_vars = {}
		self.M_vars = {}
		for va in iol.variant_type_iter():
...
def write_by_annot_varcount(by_annot_varcount_dict, outfile_mask):
	...
	with open(outfile_name, 'w') as outh:
		outh.write("Inheritance\tVariantAnnotation\tVariantCount\n")
	...
	for ind_id, ind_varcount in varcount_dict.items():
		for inher, inher_dict in ind_varcount.items():
			if not by_annot_varcount_dict.get(inher):
				by_annot_varcount_dict[inher] = iol.init_varcount_dict()
...
def load_varcount_from_file(by_annot_varcount_dict, 
	...
	with open(infile_name, 'r') as inh:
		for vc_str in inh:
			if vc_str.startswith("Inheritance"):
				continue
...
def load_by_annot_varcount_dict(input_IDs, variant_annots, suppress_indels_bool):
	...
	# variant_annots: variant annotations specified by the user via the --variant_annots
	# suppress_indels_bool: bool specified by the user
...
def write_varcount_dist(varcount_dict, outfile_mask, var_annots, score_thr_dict):
	# varcount_dict: individual ID (file name) -> variant annotation -> # variants
	# outfile_mask: identifier of the output file specified by the user
	# var_annots: variant annotations specified by the user via the --variant_annots
	#	parameter of one of the master scripts
	# score_thr_dict: score thresholds specified by the user
	...
	outfile_name = f"{outfile_mask}_{cfg.varcount_mask}.txt"
	...
	with open(f"{outfile_name}", 'w') as outh:
		...
		outh.write("Inheritance\tvariant_annotation\tvariant_count\tfrequency\n")
...
# Updates the already initialized Gene_inst_dict
# Is called by the load_muttargs_from_filelist function
def load_muttargs_from_file(Gene_inst_dict, 
							input_ID, 
							variant_annots, 
							suppress_indels_bool):
	# Gene_inst_dict: variant annotation -> ENS_ID -> Gene object
	...
	with open(infile_name, 'r') as inh:
		...
		discarded_vars = 0
		for mu_str in inh:
			if mu_str.startswith("variant_annot"):
				continue
			mu_str = mu_str.strip().split()
			va, ENS_ID, muttargs = mu_str[0], mu_str[1], mu_str[2]
			muttargs = [eval(i) for i in muttargs.split(',')]
			va = (va[0], va[1])
			# Filter by specified annotations
			if not va[0] in variant_annots:
				continue
			if suppress_indels_bool and va[1] == 'I':
				continue
			for mu in muttargs:
				# Filter by genes accepted in Gene_inst_dict
				if not Gene_inst_dict[va].get(ENS_ID):
					discarded_vars += 1
					continue
				# Gene_inst_dict update
				Gene_inst_dict[va][ENS_ID].mu_list.append(mu)
				accepted_vars += 1
...
#	metadata run.
def write_muttargs(Gene_inst_dict, outfile_mask):
	# Gene_inst_dict: variant annotation -> ENS_ID -> Gene object
	# outfile_mask: identifier of the output file specified by the user
	outfile_name = f"{outfile_mask}_{cfg.muttargs_list_DN_ID}.txt"
	with open(outfile_name, 'w') as outh:
		outh.write(f"variant_annot\tENS_ID\tmuttarg_list\n")
...
def make_gene_mutdict(Gene_inst_dict):
	...
	# gene_mutdict: ENS_ID -> list of Variant instances
	gene_mutdict = {}
	for va, va_Gene_obj_dict in Gene_inst_dict.items():
		for ENS_ID, Gene_obj in va_Gene_obj_dict.items():
			for Variant_obj in Gene_obj.vars:
...
def calc_DN_lambda(varcount_dict, Gene_inst_dict, ENS_ID):
	...
	# Gene_inst_dict: variant annotation -> ENS_ID -> Gene object
	# ENS_ID: ENSEMBL ID of the focal gene
	lam = 0
	for va in cfg.var_annot_list:
...
def generate_output_line(P_arr, 
						 outh, 
						 Gene_inst_dict, 
						 ENS_ID, 
						 ENS2GeneID_dict, 
						 gene_mutdict, 
						 first_line = False):
	# P_arr: process_single_gene output
	# outh: output file object
	# Gene_inst_dict:  variant annotation -> ENS_ID -> Gene object
	# ENS_ID: gene ENSEMBL ID
	# ENS2GeneID_dict: a dictionary ensembl_id_dict: ENS_ID -> Gene ID
	...
	if gene_mutdict:
		...
		ind_list = [Variant_obj.proband_id for Variant_obj in gene_mutdict[ENS_ID]]
		ind_list = ','.join(ind_list)
		# List of variant information
		var_info_list = [Variant_obj.print_info() for Variant_obj in gene_mutdict[ENS_ID]]
...
def wFDR(Q_arr, FDR_arr, N_genes):
	...
	# N_genes: Number of processed genes
	Q_arr = sorted(Q_arr)
	l_Q_arr = len(Q_arr)
...
def FDR_procedure(outfile_mask, FDR_arr, N_genes):
	...
	with open(filename, 'r') as inh:
		...
		for output_str in inh:
			...
			if output_str[0] == "Ind_list":
				# Header init
				h = {output_str[i]:i for i in range(len(output_str))}
				header_str = output_str
				continue
			Q_val = eval(output_str[h["Q_val"]])
...
def calc_DN_stat(ENS_ID_y_dict, 
	...
	#	in s_het deciles
	s_het_bins = iol.load_s_het_bins()

	# ENS2GeneID_dict: ENS_ID -> Gene ID
	...
	with open(filename, 'w') as outh:
		# Header is printed
		generate_output_line(P_arr = None, 
							 outh = outh, 
...
def write_mutnum_prods(mutnum_prod_dict, 
	...
	with open(filename, 'w') as outh:
		outh.write("VariantAnnot_P\tVariantAnnot_M\tcount\n")
	...
	#	intermediate output
	with open(filename, 'w') as outh:
		outh.write("VariantAnnot_P\tVariantAnnot_M\tcount\tfrequency\n")
...
def mutnum_prod(varcount_dict, outfile_mask, consequence_list):
...
def read_mutnum_prods_from_file(file_id, 
	...
	# suppress_indels_bool: specified by the user if the indels are
	#	not to be processed.

	filename = f"{file_id}_{cfg.mutnum_prod_CH}.txt"
	with open(filename, 'r') as inh:
		for mnp_str in inh:
			if mnp_str.startswith("VariantAnnot_P"):
				continue
...
def write_CH_muttargs(CH_dict, outfile_mask):
	...
	with open(filename, 'w') as outh:
		outh.write(f"variant_annot\tENS_ID\tmuttargs\n")
	...
	print(f"CH variant mutational targets written to: {filename}")
...
def read_CH_muttargs_from_file(file_id, 
	...
	print("read_CH_muttargs_from_file", filename)
	with open(filename, 'r') as inh:
		for mt_line in inh:
			if mt_line.startswith("variant_annot"):
				continue
			mt_line = mt_line.strip().split()
			ENS_ID = mt_line[1]
			mu = eval(mt_line[2])
			annots = mt_line[0].split(',')
			if annots[0][0] not in variant_annots or annots[1][0] not in variant_annots:
				continue

			if suppress_indels_bool and (annots[0][1] == 'I' or annots[1][1] == 'I'):
				continue
			if not CH_y_dict.get(ENS_ID):
				CH_y_dict[ENS_ID] = 0
			CH_y_dict[ENS_ID] += 1 - mu
...
def calc_CH_y_dict(CH_dict):
	# CH_dict: ENS_ID -> VariantCollection object
...
def generate_output_line_CH(P_arr, 
	...
	# P_arr: truncated process_single_gene output (without s_het bins and
...
#	Gene_inst_dict.
# Used to re-use the initially built Gene_inst_dict multiple times
